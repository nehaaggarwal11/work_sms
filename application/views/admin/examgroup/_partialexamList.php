...
if ($this->rbac->hasPrivilege('exam_assign_view_student', 'can_view')) {
   ...
   title = "<?php echo $this->lang->line('assign / view'); ?>" class="btn btn-default btn-xs assignStudent"  id="load" data-examid="<?php echo $exam_value->id; ?>" ><i class="fa fa-tag"></i> Assign</button>
		 	<?php
}
    if ($this->rbac->hasPrivilege('exam_subject', 'can_view')) {
        ?>
		 	 <button class="btn btn-default btn-xs" id="subjectModalButton" data-toggle="tooltip"   data-exam_id="<?php echo $exam_value->id; ?>"  title="<?php echo $this->lang->line('exam') . " " . $this->lang->line('subjects') ?>"><i class="fa fa-book" aria-hidden="true"></i> Add/View Subjects</button>
		 	<?php
}
    if ($this->rbac->hasPrivilege('exam_marks', 'can_view')) {
        ?>
		 	<button type="button" class="btn btn-default btn-xs examMarksSubject" id="load" data-toggle="tooltip"  data-recordid="<?php echo $exam_value->id; ?>" title="<?php echo $this->lang->line('exam_marks'); ?>" data-loading-text="<i class='fa fa-spinner fa-spin'></i>"><i class="fa fa-newspaper-o"></i> Add Marks</button>
		 	<?php
}
