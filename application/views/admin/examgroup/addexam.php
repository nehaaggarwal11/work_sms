...
<script>
    $(document).on('click', '.add', function() {
        ...
        html += '<td class="col-sm-3"><select name="subject_' + x + '" class="form-control item_unit" required>' + batch_subjects + '</select></td>';
        html += '<td class="col-sm-2"><div class="input-group "><input  type="text" name="date_from_' + x + '" class="form-control date_from_today" required/><span class="input-group-addon" id="basic-addon2"><i class="fa fa-calendar"></i></span></div></td>';
        html += '<td class="col-sm-2"><div class="input-group datepicker_init_time"><input type="text" name="time_from' + x + '" class="form-control" required/><span class="input-group-addon" id="basic-addon2"><i class="fa fa-calendar"></i></span></div></td>';

        html += '<td class="col-sm-1"><input type="text" name="duration' + x + '" class="form-control duration" value="0" required/></td>';
        html += '<td class="col-sm-1"><input type="number" name="credit_hours' + x + '" class="form-control credit_hours" value="0" required/></td>';
        html += '<td class=""><input type="text" name="room_no_' + x + '" class="form-control room_no" /></td>';
        html += '<td class=""><input type="number" name="max_marks_' + x + '" class="form-control max_marks" required/></td>';
        html += '<td class=""><input type="hidden" name="rows[]" value="' + x + '"> <input name="prev_row[' + x + ']" type="hidden" value="0"><input type="number" name="min_marks_' + x + '" class="form-control min_marks" required/></td>';
        html += '<td class="text-center" style="vertical-align: middle; cursor: pointer;"><span class="text text-danger remove glyphicon glyphicon-remove"></span></td></tr>';
    ...
    $(document).ready(function() {
        $(document).on('click', "#btnSubmit", function(event) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    $('#sdh_alert_success').html('<div id="toast-container" class="toast-top-right"><div class="toast toast-success" style=""><button class="toast-close-button">×</button><div class="toast-message"> Successfully Uploaded.</div></div></div>').fadeOut(4000);
