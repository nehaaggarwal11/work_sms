...
class Question_model extends MY_model {
    ...
    public function addBulk($sdh_subject_id, $sdh_question, $sdh_opt_a, $sdh_opt_b, $sdh_opt_c, $sdh_opt_d, $sdh_opt_e, $sdh_correct){
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false);
        for($in = 0; $in < count($sdh_question);$in++){
             $q1 = $sdh_subject_id[0]; 
             $q2 = $sdh_question[$in];
             $q3 = $sdh_opt_a[$in];
             $q4 = $sdh_opt_b[$in];
             ...
             $q7 = $sdh_opt_e[$in];
             $q8 = $sdh_correct[$in];
            $result = $this->db->query("INSERT INTO `questions`(`subject_id`, `question`, `opt_a`, `opt_b`, `opt_c`, `opt_d`, `opt_e`, `correct`) VALUES('$q1', '$q2', '$q3', '$q4', '$q5', '$q6', '$q7','$q8')");
        }
             $this->db->trans_complete(); # Completing transaction
             if ($this->db->trans_status() === false) {
                 # Something went wrong.
       ...
       // return $this->db->insert_batch('questions', $data);  
             //
            // return $this->db->get_compiled_insert('questions', $data);       
        // $id = $this->db->insert_id();
        // $message      = INSERT_RECORD_CONSTANT . " On  questions id " . $id;
        // $action       = "Insert";
        // $record_id    = $id;
